...
//to update status of sector when sector is written
...
*/
    ...
    if(disk_info.res_sector<sect_no<(disk_info.res_sector+2))
        {
...
void DiskHandle::delete_file(const char file[],const char f2[])
{
    ...
    outfile.seekp((disk_info.dir_sec+i)*disk_inf.size_of_sector);
    outfile.write(bufferin,disk_info.size_of_sector);
    outfile.close();
